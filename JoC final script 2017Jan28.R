...
## R SCRIPT as of Jan 28th, 2017 (JoC final accepted version).
## THIS SCRIPT ASSUMES THAT YOU HAVE NETLOGO 6.0 AND RNETLOGO 1.0-3 INSTALLED
## THE OTHER VERSION OF NETLOGO AND RNETLOGO MAY NOT WORK WITH THIS SCRIPT
## IT IS YOUR RESPONSIBILITY TO VERIFY WHETHER YOU HAVE A PROPER VERSION OF SOFTWARES INSTALLED
...
# modify following path where appropriate
   ...
   nl.path <-  "/Applications/NetLogo 6.0/Java"
   model.path.0 <- "/Users/songh95/Dropbox/GitHub/ABM/Model/Model 5.nlogo"
...
parLapply(cl, 1:processors, pre_process, gui = gui, nl.path = nl.path, model.path = model.path.0, nl.obj = "main.models")

## model estimation
result.par.model1 <- parLapply(cl, rand.seed, sim_model1, nl.obj = "main.models")
result.par.model1 <- lapply(1:100, function(k) { process.output(result.par.model1[[k]]) })

result.par.model2 <- parLapply(cl, rand.seed, sim_model2, nl.obj = "main.models")
result.par.model2 <- lapply(1:100, function(k) { process.output(result.par.model2[[k]]) })

result.par.model3 <- parLapply(cl, rand.seed, sim_model3, nl.obj = "main.models")
result.par.model3 <- lapply(1:100, function(k) { process.output(result.par.model3[[k]]) })

result.par.model4 <- parLapply(cl, rand.seed, sim_model4, nl.obj = "main.models")
result.par.model4 <- lapply(1:100, function(k) { process.output(result.par.model4[[k]]) })

result.par.model5 <- parLapply(cl, rand.seed, sim_model5, nl.obj = "main.models")
result.par.model5 <- lapply(1:100, function(k) { process.output(result.par.model5[[k]]) })

result.par.model6 <- parLapply(cl, rand.seed, sim_model6, nl.obj = "main.models")
...
## load Netlogo in each processor/core for additional models
parLapply(cl, 1:processors, pre_process, 
          gui = gui, nl.path = nl.path, nl.obj = "model4_strong", model.path = model.path.1)
## model estimation for additional models
results.par.model4.strong.attitudes <- parLapply(cl, rand.seed, sim_model4_strong_attitudes, nl.obj = "model4_strong")
results.par.model4.strong.attitudes <- lapply(1:100, function(k) { process.output(results.par.model4.strong.attitudes[[k]]) })

# change the model
parLapply(cl, 1:processors, pre_process, 
          gui = gui, nl.path = nl.path, nl.obj = "model4_indirect", model.path = model.path.2)
...
parLapply(cl, 1:processors, pre_process, 
          gui = gui, nl.path = nl.path, nl.obj = "model4_european", model.path = model.path.2)
...
parLapply(cl, 1:processors, pre_process, 
          gui = gui, nl.path = nl.path, nl.obj = "model4_interest_interaction", model.path = model.path.2)
...
parLapply(cl, 1:processors, pre_process, nl.obj = "model4_information_decay",
...
R2.models <- list(results.par.model4.strong.attitudes, results.par.model6.strong.attitudes,
                  ...
                  results.par.model4.information.decay, results.par.model6.information.decay)
