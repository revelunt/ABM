...
to go
  ...
  if count turtles with [ attitudes > -3 ] = 0 or count turtles with [ attitudes < 3 ] = 0 [stop] ;; if one of the partisan camp disappears completely, the model stops
  if ticks >= 500 [stop] ;; after 200 iteration, model stops
...
to watch-media  ;; how "selective" exposure map into this submodel?
  ...
  [ ifelse attitudes < 0 ;; if republicans
  [
    set blue-media random (abs(attitudes) + 1)

    if exposure-to-counter-media < 1  ;; (global) if exposure-to-counter-media parameter is less than 1, do following...
    [ set blue-media (blue-media * exposure-to-counter-media) ]
  ...
  [ ;; if neutral
    ifelse attitudes = 0
    [ set red-media random 4
      set blue-media random 4  ;; randomly get blue and red media exposure, to varying (random) degree from 0 to 3

      if exposure-to-counter-media < 1  ;; (global) if exposure-to-counter-media parameter is less than 1, do following...
      [ set red-media (red-media * exposure-to-counter-media)
        set blue-media (blue-media * exposure-to-counter-media) ]
    ]

  [ ;; finally, for democrats,

    set red-media random (abs(attitudes) + 1)

    if exposure-to-counter-media < 1  ;; (global) if exposure-to-pro-media parameter is less than 1, do following...
      [ set red-media (red-media * exposure-to-counter-media) ]
...
[
   ...
   let total count my-connected-neighbors
   let rep (count my-connected-neighbors with [ attitudes < 0 ])
   let ind (count my-connected-neighbors with [ attitudes = 0 ])
   let dem (count my-connected-neighbors with [ attitudes > 0 ])
   set diversity 1 - ((rep / total) * (rep / total) - (ind / total) * (ind / total) - (rep / total) * (rep / total))
]
...
to update-opinion  ;; need to work on this logic !!!!!!!!!!!!!!
  ...
  let attitudes_t-1 attitudes
  let SINP social-influence-parameter
  let MINP media-influence-parameter
  let STBP stability-parameter
  set prior-attitudes attitudes * STBP  ;; if republican, a value of attitude increases. if democrat, a value of prior attitude decreases (randomly regresses to neutral point).

if opinion-update-model? = "weighted-mean-average with disagreement"

[   ifelse any? my-connected-neighbors
  [ ;; if there's connected neighbors

    if attitudes < 0  ;; for Republicans
    [ set tally (prior-attitudes + (MINP * (disagree) * blue-media - MINP * (1 - disagree) * red-media) + SINP * total_social) ]

    if attitudes > 0  ;; for democrats
    [ set tally (prior-attitudes + (MINP * (1 - disagree) * blue-media - MINP * (disagree) * red-media) + SINP * total_social) ]

    if attitudes = 0 ;; for independents
    [ set tally (prior-attitudes + (MINP * (disagree) * blue-media - MINP * (disagree) * red-media) + SINP * total_social) ] ]

  [ ;; if there's no connected neighbors

    set tally (prior-attitudes + MINP * blue-media - MINP * red-media) ] ;; for those who have no connected neighbors, social influence is not considered..

  ;; finally, update attitudes
  if tally < attitudes_t-1 [set attitudes attitudes_t-1 - 1]
  if tally > attitudes_t-1 [set attitudes attitudes_t-1 + 1]
...
[ ifelse any? my-connected-neighbors
  [ ;; if there's connected neighbors
    ...
    set tally (prior-attitudes + (MINP * blue-media - MINP * red-media) + SINP * total_social) ]
...
if opinion-update-model? = "test"

[ ifelse any? my-connected-neighbors
  [ ;; if there's connected neighbors

    set tally (prior-attitudes + MINP * (1 - disagree) * total_media + SINP * total_social) ]

  [ ;; if there's no connected neighbors

    set tally (prior-attitudes + MINP * (1 - disagree) * total_media) ]
...
"" ""
PENS
"at-least-one-disagree" 1.0 0 -16777216 true "" "if any? turtles with [ disagree > 0 ] [plot mean [ disagree ] of turtles with [ disagree > 0 ]]"
"overall" 1.0 0 -7500403 true "" "plot mean [ disagree ] of turtles"
...
opinion-update-model?
"weighted-mean-average with disagreement" "weighted-mean-average" "test"
0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean [total_media] of turtles"
...
1316
629
mean disagreement, overall
mean [ disagree ] of turtles
17
1
11
...
PENS
"default" 2.0 1 -16777216 true "" "histogram [attitudes] of turtles"
...
"" ""
PENS
"Dem" 1.0 0 -14454117 true "" "if any? turtles with [ attitudes > 0 ] [plot mean [attitudes] of turtles with [attitudes > 0]]"
"Rep" 1.0 0 -5298144 true "" "if any? turtles with [ attitudes < 0 ] [plot mean [attitudes] of turtles with [attitudes < 0]]"
"overall" 1.0 0 -7500403 true "" "plot mean [attitudes] of turtles"
